...
function init() {
  ...
  for (let j = 0; j < curveAmount; j++) {
    ...
    for (
      let i = j * splinePointsLength;
      i < (j + 1) * splinePointsLength;
      i++
    ) {
...
function addTrajectory() {
  ...
  for (let i = 0; i < folderNames.length; i++) {
    ...
    let curveController =
      currentFolder.controllers[currentFolder.controllers.length - 1]; // Get the last controller
...
function deleteTrajectory() {
  if (curves.length <= 0) {
    // Check if there are any curves to delete
    return;
  }
  ...
  for (let i = 0; i < splinePointsLength; i++) {
    let lastSplineHelperObject =
      splineHelperObjects[splineHelperObjects.length - 1];
...
function removePoint(curveIndex) {
  ...
  if (controlPointfolderNames.length > 0) {
    ...
    let lastcontrolPointfolderName =
      controlPointfolderNames[controlPointfolderNames.length - 1];
